...
static const int   MAX_DEPTH = 6;  //how many bounces the light path may take
...
CRTVector shade(const CRTScene& scene, const CRTVector& P, const CRTVector& N, const CRTVector& albedo) {
    CRTVector shadowOrigin = P + N * BIAS;
    CRTVector col(0, 0, 0);
...
CRTVector trace(const CRTScene& scene, const CRTVector& origin, const CRTVector& I, int depth) {
    ...
    float t;
    int idx = closestHit(scene, origin, I, t);
    if (idx < 0) return bg;
    ...
    if (mat.reflective) {
        CRTVector R = reflect(I, N);
        return mulc(trace(scene, P + N * BIAS, R, depth + 1), mat.albedo);
    }

    if (mat.refractive) {
        float eta1 = 1.0f, eta2 = mat.ior;//outside = air, inside = glass
        CRTVector n = N;
        float dotIN = I.dot(n);
        if (dotIN > 0.0f) { //ray is leaving the object
            n = n * -1.0f;
            std::swap(eta1, eta2);
            dotIN = I.dot(n);
        }
        float cosI = -dotIN; // between I and the front normal (>0)
        float eta  = eta1 / eta2;

        CRTVector R = reflect(I, n);
        CRTVector reflCol = trace(scene, P + n * BIAS, R, depth + 1);

        float k = 1.0f - eta * eta * (1.0f - cosI * cosI);
        if (k < 0.0f) return reflCol;  // only reflection

        float cosR = std::sqrt(k);
        CRTVector refrDir = (I * eta + n * (eta * cosI - cosR)).normalize();

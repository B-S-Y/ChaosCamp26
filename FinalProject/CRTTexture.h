...
#include <vector>
#include <cmath>
#include "stb_image.h"     // implementation is defined once, in task1.cpp
#include "CRTVector.h"

// A texture the albedo is read from. Four kinds:
//  - albedo : a constant color
//  - edges  : edge color near the triangle edges, inner color otherwise
//  - checker: a checkerboard from the UV coordinates
//  - bitmap : sample a loaded image by UV
struct CRTTexture {
    ...
    // Load a bitmap image (jpg/png) into memory (3 channels).
    ...
    // Return the albedo color at the hit (bary coords + interpolated UV).

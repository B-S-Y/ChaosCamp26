...
struct CRTCamera {
    CRTVector position;   // where the camera is
    CRTMatrix rotation;   // where it looks (identity = looking down -Z)

    CRTCamera() : position(0.0f, 0.0f, 0.0f) {}                 // default: origin
    CRTCamera(const CRTVector& pos) : position(pos) {}          // custom position

    // --- Rotations (angles in degrees) ---
    // Multiplying on the right rotates around the camera's own (local) axes.
    ...
    // --- Translations (distance in world units, along local axes) ---
    ...
    // Turn a camera-space direction into a world-space ray direction.

...
struct CRTScene {
    ...
    bool load(const std::string& filename) {
        ...
        // Folder that holds this scene (used to resolve texture file paths).
        std::string baseDir;
        ...
        if (doc.HasMember("lights")) {
            ...
            for (rapidjson::SizeType i = 0; i < ls.Size(); ++i)
                lights.push_back(CRTLight(readVec(ls[i]["position"]), ls[i]["intensity"].GetFloat()));
        }
        ...
        if (doc.HasMember("textures")) {
            ...
            for (rapidjson::SizeType i = 0; i < ts.Size(); ++i) {
                CRTTexture tex;
                tex.name = ts[i]["name"].GetString();
                std::string type = ts[i]["type"].GetString();
                if (type == "albedo") {
                    tex.type = CRTTexture::ALBEDO;
                    tex.albedo = readVec(ts[i]["albedo"]);
                } else if (type == "edges") {
                    tex.type = CRTTexture::EDGES;
                    tex.edgeColor  = readVec(ts[i]["edge_color"]);
                    tex.innerColor = readVec(ts[i]["inner_color"]);
                    tex.edgeWidth  = ts[i]["edge_width"].GetFloat();
                } else if (type == "checker") {
                    tex.type = CRTTexture::CHECKER;
                    tex.colorA = readVec(ts[i]["color_A"]);
                    tex.colorB = readVec(ts[i]["color_B"]);
                    tex.squareSize = ts[i]["square_size"].GetFloat();
                } else if (type == "bitmap") {
                    tex.type = CRTTexture::BITMAP;
                    std::string fp = ts[i]["file_path"].GetString();
                    if (!fp.empty() && (fp[0] == '/' || fp[0] == '\\')) fp = fp.substr(1);
                    tex.loadBitmap(baseDir.empty() ? fp : baseDir + "/" + fp);
                }
        ...
        if (doc.HasMember("materials")) {
            ...
            for (rapidjson::SizeType i = 0; i < ms.Size(); ++i) {
                ...
                if (ms[i].HasMember("albedo")) {
                    ...
                    if (a.IsString()) m.textureIndex = findTexture(a.GetString());
                    else if (a.IsArray()) m.albedo = readVec(a);
                }
